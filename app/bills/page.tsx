...
export default function BillsPage() {
  ...
  const buildInvoiceHTML = (inv: InvoiceDetail) => {
    const calcGrandTotal = inv.invoice_items.reduce((acc, item) => acc + item.price * item.quantity * (1 - item.discount / 100), 0);
    const calcDiscount = inv.invoice_items.reduce((acc, item) => acc + item.price * item.quantity * (item.discount / 100), 0);
    const calcNetTotal = inv.invoice_items.reduce((acc, item) => acc + item.price * item.quantity, 0);
    ...
    return `<div id="invoice-pdf-content" style="font-family:Arial,sans-serif;margin:0;padding:0;background:#fff;"><div style="width:95%;margin:20px auto;padding:10px;border:1px solid #000;"><div style="text-align:center;font-size:22px;font-weight:bold;margin-bottom:5px;text-transform:uppercase;">Anuradha Transport Services</div><div style="text-align:center;font-size:12px;margin-bottom:15px;">No.219, Nawana, Mirigama<br/>Tel: 0777 898 929 / 0727 898 929 / 0770 584 959</div><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:10px;"><div><strong>Invoice No:</strong> ${inv.invoice_code}</div><div><strong>Date:</strong> ${dateStr}</div></div><div style="font-size:12px;border:1px solid #000;padding:8px;margin-bottom:10px;"><strong>Brown &amp; Company PLC - Pharmaceuticals Division</strong><br/>34, Sir Mohamed Macan Marker Mawatha, Colombo 03<br/>Tel: 011 266 3000</div><div style="font-size:12px;border:1px solid #000;padding:8px;margin-bottom:10px;"><strong>Customer Name:</strong> ${inv.customers?.name || "N/A"}<br/><strong>Address:</strong> ${inv.customers?.address || "N/A"}<br/><strong>Contact No:</strong> ${inv.customers?.contact || "N/A"}</div><table style="width:100%;border-collapse:collapse;font-size:12px;"><thead><tr style="background-color:#e0e0e0;"><th style="border:1px solid #000;padding:6px;text-align:left;text-transform:uppercase;">Qty</th><th style="border:1px solid #000;padding:6px;text-align:left;text-transform:uppercase;">Description</th><th style="border:1px solid #000;padding:6px;text-align:left;text-transform:uppercase;">Discount</th><th style="border:1px solid #000;padding:6px;text-align:left;text-transform:uppercase;">Unit Price (LKR)</th><th style="border:1px solid #000;padding:6px;text-align:left;text-transform:uppercase;">Amount (LKR)</th></tr></thead><tbody>${rows}${emptyRows}<tr><td colspan="3" style="border:none;"></td><td style="border:1px solid #000;padding:6px;"><strong>Sub Total</strong></td><td style="border:1px solid #000;padding:6px;">LKR ${calcNetTotal.toFixed(2)}</td></tr><tr><td colspan="3" style="border:none;"></td><td style="border:1px solid #000;padding:6px;"><strong>Discount</strong></td><td style="border:1px solid #000;padding:6px;">LKR ${calcDiscount.toFixed(2)}</td></tr><tr><td colspan="3" style="border:none;"></td><td style="border:1px solid #000;padding:6px;"><strong>Grand Total</strong></td><td style="border:1px solid #000;padding:6px;"><strong>LKR ${calcGrandTotal.toFixed(2)}</strong></td></tr></tbody></table><div style="margin-top:15px;font-size:12px;border-top:1px solid #000;padding-top:8px;">PLEASE DRAW THE CHEQUE IN FAVOUR OF ANURADHA TRANSPORT SERVICES</div><div style="margin-top:30px;display:flex;justify-content:space-between;font-size:12px;"><span>Checked By: ________________________</span><span>Goods Received By Customer</span></div></div></div>`;
  };
